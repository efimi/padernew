...
def searchIt(searchKey):
	...
	for place in query_result.places:
		...
		Locations.create(name=place.name, address=place.formatted_address, phone=place.international_phone_number, email="", lat=lat, lng=lng, token=token, website=place.website, maps_url=place.url, category=searchKey)
		if 'opening_hours' in str(outputJson):
			for period in outputJson['opening_hours']['periods']:
				...
				if 'open' in period:
					...
					otime = "no"
				if cday == oday:
					Opening_Hours.create(location_id = i, day_id = cday, closed = ctime, opened = otime)
				else:
					Opening_Hours.create(location_id = i, day_id = oday, closed = "" , opened = otime)
					Opening_Hours.create(location_id = i, day_id = cday, closed = ctime, opened = "")

	
		for photo in place.photos:
			j = 1
			photo.get(maxheight=500, maxwidth=500)
			Photos.create(location_id=i,name="%s-%d" % (place.name,j),url=photo.url)
			j = j + 1 
		i = i + 1 
